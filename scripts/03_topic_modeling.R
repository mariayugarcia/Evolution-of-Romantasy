...
print(
  ggplot(coherencias, aes(K, mean_coherence)) +
    ...
    labs(
      title = "UMass coherence by number of topics (standard LDA)",
...
topic_labels <- c(
  "1" = "Supernatural conflict",
  "2" = "Underworld",
  "3" = "Personal drama",
  "4" = "Relationships and bonds",
  "5" = "Council and institutions"
)
...
chapters_text_seed <- books_with_chapters_shadowhunters %>%
  dplyr::filter(nchar(trimws(text)) > 0) %>%
  dplyr::mutate(text = stringr::str_replace_all(text, "[’`]", "'")) %>%  # <-- CLAVE
  dplyr::group_by(book, chapter_num) %>%
  dplyr::summarise(
    chapter_text = paste(text, collapse = " "),
    .groups = "drop"
