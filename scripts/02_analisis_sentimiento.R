...
g1 <- ggplot(evolucion, aes(chapter_num, sent_mean, color = saga)) +
  ...
  labs(
    x = "Chapter number (relative)",
    y = "Average Score AFINN",
    title = "Evolution of emotional sentiment by saga"
  ) +
  theme_economist() +
  theme(
...
g2 <- ggplot(comparacion, aes(sentiment, media_prop, fill = saga)) +
  ...
  labs(
    x = "Emotion",
    y = "Average proportion by chapter",
    title = "Emotional comparison between sagas"
  )  +
  theme_economist() +
  theme(
    plot.title = element_text(face = "bold", size = 16, hjust = 0.5),
...
g3 <- ggplot(emocion_por_saga, aes(x = chapter_rel, y = prop_media, color = saga)) +
  ...
  labs(
    x = "Chapter (progress within the saga)",
    y = "Average proportion of words with the emotion",
    title = "Evolution of emotions by saga",
    color = "Saga"
  ) +
  theme_economist() +
  theme(
...
g4 <- ggplot(sentiment_by_window, aes(x = index, y = sentiment_net, fill = book)) +
  ...
  labs(
    x = glue::glue("Segments of {WINDOW_SIZE} lines"),
    y = "Net sentiment (positive − negative)",
    title = "Evolution of sentiment in selected novels"
  ) +
  theme_economist() +
  theme(
    plot.title = element_text(face = "bold", size = 16, hjust = 0.5),
...
g5 <- ggplot(top_emotion_words_by_saga, aes(n, word, fill = saga)) +
  ...
  labs(
    ...
    y = NULL,
    title = "Top 10 words by emotion: comparison between sagas",
    fill = "Saga"
  ) +
  theme_economist() +
  theme(
...
g6 <- ggplot(top_emotion_words, aes(n, word, fill = sentiment)) +
  ...
  labs(
    x = "Frequency in the corpus",
    y = NULL,
    title = "Words that most activate each emotion (NRC - Plutchik)"
  ) +
  theme_economist() +
  theme(
    plot.title = element_text(face = "bold", size = 16, hjust = 0.5),
...
g7 <- ggplot(valencia_combinada_long, aes(chapter_num, puntuación, color = método)) +
  ...
  labs(
    title = "Emotional valence comparison: AFINN vs NRC",
    x = "Chapter",
    y = "Sentiment score",
    color = "Method"
  ) +
...
chapters_empyrean <- books_with_chapters_shatterme_empyrean %>%
  filter(saga %in% "Empyrean") %>%
  group_by(book, chapter_num) %>%
  summarise(text_full = paste(text, collapse = " "), .groups = "drop") %>%
  mutate(text_full = clean_text_for_model(text_full)) %>%
  filter(nchar(text_full) > 100)
...
p1 <- ggplot(comparison_long, aes(chapter_num, sentiment, color = method)) +
  ...
  labs(
    x = "Chapter",
    y = "Sentiment",
    title = "Comparison: custom model vs Bing in Empyrean"
  ) +
  theme_economist() +
  theme(
    plot.title = element_text(face = "bold", size = 16, hjust = 0.5),
...
books_with_chapters_empyrean <- books_with_chapters_shatterme_empyrean %>%
  filter(saga %in% "Empyrean")

# Tabla comparativa final
all_chapters <- bind_rows(
  books_with_chapters_empyrean %>% mutate(corpus = "Empyrean"),
...
p2 <- ggplot(words_impact_emotional,
  ...
  labs(
    x = "Weight in the model",
    y = NULL,
    title = "Most influential emotional words in the classifier"
  ) +
  theme_economist() +
  theme(
    plot.title = element_text(face = "bold", size = 16, hjust = 0.5),
...
p3 <- ggplot(sentimentr_results_all, aes(x = sentiment, fill = saga)) +
  ...
  labs(
    x = "Emotional tone (sentimentr: average per chapter)",
    y = "Density",
    title = "Distribution of emotional tone by saga"
  ) +
  theme_economist() +
  theme(
    plot.title = element_text(face = "bold", size = 16, hjust = 0.5),
