...
g1 <- ggplot(metricas_long, aes(x = saga, y = valor, fill = métrica)) +
  ...
  labs(
    title = "Narrative complexity: Shadowhunters vs Empyrean",
    x = NULL,
    y = "Quantity",
    fill = "Metric"
  ) +
...
g2 <- ggplot(cent_emp, aes(x = personaje, y = centralidad)) +
  ...
  coord_flip() +
  labs(
    title = "Character centrality in Empyrean",
    x = NULL,
...
g3 <- ggplot(cent_sha, aes(x = personaje, y = centralidad)) +
  ...
  coord_flip() +
  labs(
    title = "Character centrality in Shadowhunters (The Mortal Instruments)",
    x = NULL,
    y = "Degree (number of connections)"
  ) +
  theme_economist() +
